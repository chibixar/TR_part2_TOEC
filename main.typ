...
#mathtype-mimic[
  $ dot(U)_32 = -dot(I)_"конт" dot(Z)_6 + dot(E)_(J_6) = \
              = -(-0.976 + 0.779 j)(-30 j) + (-174.09 + 117.42 j) = \
              = -197.46 + 88.14 j " В". $
]
Ток 6-й ветви по закону Ома:
#mathtype-mimic[
  $ dot(I)_6 = dot(U)_32 / dot(Z)_6 = (-197.46 + 88.14 j) / (-30 j) = \
             = -2.938 - 6.582 j = 7.208 e^(-j 114.05 degree) " А". $
]
...
#mathtype-mimic[
  $ dot(U)_(J_6) = (dot(I)_"конт" - dot(J)_6) dot dot(Z)_6 = \
...
#mathtype-mimic[
  $ dot(U)_"хх" = dot(J)_6 dot dot(Z)_"экв" - dot(E)_5 = \
                = (-3.914 - 5.800 j) (194.56 + 29.43 j) - (-15.46 + 34.71 j) = \
                = (-590.77 - 1243.33 j) - (-15.46 + 34.71 j) = \
                = -575.31 - 1279.04 j " В". $
]
